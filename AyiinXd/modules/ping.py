...
async def _(pong):
    ...
    await kopong.edit(
        ...
        f"\n ★ 𝗬𝗼𝘂𝗿 𝗣𝗶𝗻𝗴 `%sms` \n"
        f"★ 𝗨𝗽𝘁𝗶𝗺𝗲 "
        f"\n ★ User - smuddle『[{user.first_name}](tg://user?id={user.id})』 \n" % (duration)
    )
...
async def _(pong):
    ...
    await kping.edit("8===😬D🔥")
    await kping.edit("8====D🔥🔥")
    await kping.edit("**SMUDDLE**")
    await kping.edit("**DISINI 𝗦𝗺𝘂𝗱𝗱𝗹𝗲-𝗨𝘀𝗲𝗿𝗯𝗼𝘁 KECE BADAI SLEBEW .....**")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await kping.edit("☠️")
    sleep(3)
...
CMD_HELP.update(
    {
        "ping": f"**Plugin : **`ping`\
        \n\n  »  **Perintah :** `{cmd}ping` ; `{cmd}lping` ; `{cmd}xping` ; `{cmd}mudle`\
